...
@else
    ...
    <select class="form-control select2" name="{{ $row->field }}">
        ...
        @if(isset($options->options))
            @foreach($options->options as $key => $option)
                <option value="{{ $key }}" @if($default == $key && $selected_value === NULL) selected="selected" @endif @if($selected_value == $key) selected="selected" @endif>{{ __('options.'.strtolower($option)) }}</option>
            @endforeach
        @endif
    </select>
@endif
