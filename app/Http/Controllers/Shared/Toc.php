...
class Toc {
    ...
    public function __construct($content) {
        ...
        $this->level   = $match[1];
        // $this->text    = $match[2];
        $this->text    = array_map(function ($item) {
    ...
    public function getDataToc() {
        ...
        for ($i=0, $j=0; $i<$count; $i++, $j++) {
            ...
            if ($i === 0) {
                ...
                $toc[$i]['item_close'] = isset($level[$i+1]) && $level[$i] == $level[$i+1] or (!isset($level[$i+1]));
                $toc[$i]['list_close'] = isset($level[$i+1]) && $level[$i] >  $level[$i+1] or (!isset($level[$i+1]));
    ...
    public function getToc(): View {
        $dataToc = $this->getDataToc();

        return view('front.partials.toc', compact('dataToc'));
    }
